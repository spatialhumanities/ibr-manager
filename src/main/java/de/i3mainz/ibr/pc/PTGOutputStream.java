...
public class PTGOutputStream extends PTGStream {
	...
	public void write(PTGPoint p) throws IOException {
		...
		byte[] b = new byte[record];
		tmpout.read(b);
		PTGPoint old = new PTGPoint(b,properties);
		double norm = old.norm();
		if (norm < 0.0001 || norm > p.norm()) {
			tmpout.seek((col*size+row)*record);
	...
	public void close() throws IOException {
		...
		tmpout.seek(offset[0]-8*offset.length); // negative seek offset: set param!
