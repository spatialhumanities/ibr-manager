...
//      Abfangen des Bild Randes
    ...
    public static ArrayList<Point> randomPoints(Triangle triangle, int anzahl) {
        int i = 0;
        double xmax = triangle.getA().getX() < triangle.getB().getX() ? triangle.getB().getX() : triangle.getA().getX();
        double xmin = triangle.getA().getX() > triangle.getB().getX() ? triangle.getB().getX() : triangle.getA().getX();
        xmax = xmax <= triangle.getC().getX() ? triangle.getC().getX() : xmax;
        xmin = xmin >= triangle.getC().getX() ? triangle.getC().getX() : xmin;
        
        double ymax = triangle.getA().getY() < triangle.getB().getY() ? triangle.getB().getY() : triangle.getA().getY();
        double ymin = triangle.getA().getY() > triangle.getB().getY() ? triangle.getB().getY() : triangle.getA().getY();
        ymax = ymax <= triangle.getC().getY() ? triangle.getC().getY() : ymax;
        ymin = ymin >= triangle.getC().getY() ? triangle.getC().getY() : ymin;
        
        double zmax = triangle.getA().getZ() < triangle.getB().getZ() ? triangle.getB().getZ() : triangle.getA().getZ();
        double zmin = triangle.getA().getZ() > triangle.getB().getZ() ? triangle.getB().getZ() : triangle.getA().getZ();
        zmax = zmax < triangle.getC().getZ() ? triangle.getC().getZ() : zmax;
        zmin = zmin > triangle.getC().getZ() ? triangle.getC().getZ() : zmin;
        
        
        
        ArrayList<Point> points = new ArrayList<>();
        while (i < anzahl) {
            double xneu = Math.random() * (xmax - xmin) + xmin;
            double yneu = Math.random() * (ymax - ymin) + ymin;
            double zneu = Math.random() * (zmax - zmin) + zmin;
            Point neu = new Point(xneu, yneu, zneu);
            Point neu2 = triangle.projektion(neu);
            if (triangle.inTrieangelCheck(neu2)) {
                i++;
                points.add(neu2);
            }
        }
